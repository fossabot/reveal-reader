...
public class Main extends ListActivity implements OnGestureListener {
        ...
        if (mUpdating) {
            ...
            Thread t = new Thread() {
                ...
                public void run() {
                    ...
                    Looper.prepare();
                    String ebookDir = mSharedPref.getString(
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        if (!configChanged) {
            BOOLshowSplashScreen = mSharedPref.getBoolean("show_splash_screen",
                    true);
        ...
        if (!configChanged) {
            ...
            if (!android.os.Environment.getExternalStorageState().equals(
                ...
                Toast.makeText(this,
                        "You must have an SDCARD installed to use Reveal",
                        Toast.LENGTH_LONG).show();
            } else {
    ...
    private void refreshLibrary(final String strLibDir,
        ...
        fileCursor = managedQuery(bookUri, new String[] {
                YbkProvider.FILE_NAME, YbkProvider._ID }, null, null,
        ...
        if (!libraryDir.exists()) {
            if (!libraryDir.mkdirs()) {
                ...
                Util.sendNotification(this, (String) getResources().getText(
                        R.string.library_not_created),
                        android.R.drawable.stat_sys_warning,
                        "Couldn't make eBook Library", mNotifMgr, mNotifId++,
                        Main.class);
            ...
            for (int i = 0, dirListLen = ybkFiles.length; i < dirListLen; i++) {
                ...
                if (!fileFoundInDb) {
                    ...
                    if (lastSlashPos != -1 && lastDotPos != -1) {
                        bookName = dirFilename.substring(lastSlashPos + 1,
                                lastDotPos);
                    }
                    ...
                    if (Integer.parseInt(bookId) > 0) {
                        mListCursor = managedQuery(mBookUri, new String[] {
                                ...
                                YbkProvider.BINDING_TEXT + " is not null",
                                null, " LOWER(" + YbkProvider.FORMATTED_TITLE
                                        + ") ASC");
        ...
        while (!fileCursor.isAfterLast()) {
            ...
            if (!fileFoundInDir) {
                ...
                Uri deleteUri = ContentUris.withAppendedId(bookUri, Long
                        .parseLong(bookId));
    ...
    private void refreshBookList() {
        ...
        SimpleCursorAdapter bookAdapter = new SimpleCursorAdapter(this,
                R.layout.book_list_row, mListCursor, from, to);
        ...
        if (resultCode == RESULT_OK) {
            ...
            case YbkViewActivity.CALL_HISTORY:
                ...
                if (histCurs.moveToFirst()) {
                    long bookId = histCurs.getLong(histCurs
            ...
            case YbkViewActivity.CALL_BOOKMARK:
                ...
                Cursor bmCurs = managedQuery(ContentUris.withAppendedId(Uri
                        .withAppendedPath(YbkProvider.CONTENT_URI, "bookmark"),
                        bmId), null, null, null, null);

                if (bmCurs.moveToFirst()) {
                    histId = bmCurs.getLong(bmCurs
                            .getColumnIndex(YbkProvider._ID));
                    Intent intent = new Intent(this, YbkViewActivity.class);
            ...
            case ACTIVITY_SETTINGS:
                extras = data.getExtras();
                boolean libDirChanged = extras
                        .getBoolean(Settings.EBOOK_DIR_CHANGED);
