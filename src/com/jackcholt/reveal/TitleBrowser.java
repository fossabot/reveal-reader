...
public class TitleBrowser extends ListActivity {
	...
	protected void updateScreen() {
		...
		if (mListTitles.size() > 0) {
			ArrayAdapter<Title> titleAdapter = new ArrayAdapter<Title>(this,
					R.layout.browser_list_item, R.id.book_name, mListTitles);

			setListAdapter(titleAdapter);
		} else {
			ArrayAdapter<Category> categoryAdapter = new ArrayAdapter<Category>(
					this, R.layout.browser_list_item, R.id.book_name,
					mListCategories);
		...
		try {
			...
			if (selected instanceof Title) {
				if (mBusy) {
					...
					FlurryAgent.onError("TitleBrowser", "Download Busy",
							"WARNING");
				} else {
				...
				mBreadCrumb
						.push(new URL(mDownloadServer + "?c=" + category.id));
	...
	private void downloadTitle(final Title title) {
		...
		try {
			...
			Toast.makeText(this, R.string.ebook_download_failed_url,
		...
		final ProgressNotification progressNotification = new ProgressNotification(
				this, mNotifId++, R.drawable.ebooksmall, MessageFormat.format(
		...
		final Completion callback = new Completion() {
			...
			public void completed(boolean succeeded, String message) {
				...
				if (main != null) {
					if (succeeded) {
						if (message.endsWith("%")) {
							int percent = Integer.parseInt(message.substring(0,
									message.length() - 1));
		...
		SafeRunnable action = new SafeRunnable() {
			...
			public void protectedRun() {
				YbkService.requestDownloadBook(TitleBrowser.this,
						downloadUrlString, fileLocationString, callback);
				Toast.makeText(TitleBrowser.this,
						R.string.ebook_download_started, Toast.LENGTH_SHORT)
						.show();
		...
		ConfirmActionDialog.confirmedAction(this, R.string.title_browser_name,
				information.toString(), R.string.download, action);
	}
	...
	public void populate() {
		...
		try {
			...
			Toast.makeText(TitleBrowser.this, R.string.ebook_list_failed_url,
					Toast.LENGTH_LONG).show();
			finish();
		} catch (IOException e) {
			Toast.makeText(TitleBrowser.this, R.string.ebook_list_failed_io,
					Toast.LENGTH_LONG).show();
			finish();
		} catch (ParserConfigurationException e) {
			Toast.makeText(TitleBrowser.this, R.string.ebook_list_failed,
					Toast.LENGTH_LONG).show();
			finish();
	...
	private class ParserHandler extends DefaultHandler {
		...
		public void startElement(String namespaceURI, String tagName,
			...
			if (lowerTag.equals(mBookTag)) {
				...
				mCurrentCategory.id = Integer.parseInt(attributes
						.getValue("id"));
			}
